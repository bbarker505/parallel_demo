...
Unfortunately, `mclapply` does not work on Windows machines because its implementation relies on forking and Windows does not support forking. "Fork" is the name of the Unix call that the parent process uses to "divide" itself ("fork") into two identical processes. Windows uses a threading module for multiprocessing.
...
It looks like annual Tmax increased more in Central Oregon than along the Oregon coast over the last 30 years. Results for each year were generated much quicker by calculating trends for each year in parallel.
...
- For example, let's say we had to analyze changes in Tmax in July across all 3,0006 U.S. counties!  
