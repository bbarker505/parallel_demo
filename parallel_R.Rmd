...
Running the analysis sequentially is pretty slow. Instead, Tmax data for each year can be anlayzed in parallel. 

First, we need to create a parallel socket cluster (another word for core) using `parallel::makeCluster`, which creates a user-specified number of copies of R to run in parallel. Be warned that specifying too many cores may overload your computer and potentially crash processes and programs (including R). 
...
Unfortunately, `mclapply` does not work on Windows machines because its implementation relies on forking and Windows does not support forking. 
